...
function fillFields(contents) {
  ...
  fields.name = name;
  fields.profile_image = simple_match(
    '\\[\\\\"' + name + '\\\\",\\[null,4,null,null,null,null,\\[\\\\"([^\\\\]*)\\\\"]',
    contents
  );
  fields.contributions = simple_match(
    ',\\\\\\"(([\\d]{1,3},)?([\\d]{3},)*([\\d]{3})|([\\d]{1-3})) contributions\\\\\\"',
    contents
  );
  fields.photo_view = photo_view_match('(\\d+),\\\\"(C[\\w-]{34,40})\\\\",(\\d+)', contents);

  fields.points = simple_match(
    '(([\\d]{1,3},)?([\\d]{3},)*([\\d]{3})|([\\d]{1-3})) Points" itemprop="description"',
    contents
  );
  if (fields.points !== null) {
    ...
    fields.points = simple_match(
      '(([\\d]{1,3},)?([\\d]{3},)*([\\d]{3})|([\\d]{1-3})) Contributions" itemprop="description"',
      contents
    );
  }
